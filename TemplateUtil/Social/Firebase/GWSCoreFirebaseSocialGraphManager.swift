...
//  ChatApp
//
//  Created by Jared Sullivan and Florian Marcu on 9/15/18.
//  Copyright © 2018 Lawn and Order. All rights reserved.
//

import FirebaseFirestore

class GWSCoreFirebaseSocialGraphManager: GWSSocialGraphManagerProtocol {
    ...
    func fetchFriends(viewer: GWSUser, completion: @escaping (_ friends: [GWSUser]) -> Void) {
        fetchFriendships(viewer: viewer) { friendships in
    ...
    func removeFriendListeners() {
        ...
        allFriendListeners.removeAll()
    }

    func fetchFriendships(viewer: GWSUser, completion: @escaping (_ friendships: [GWSChatFriendship]) -> Void) {
        if isFriendsUpdateNeeded {
            removeFriendListeners() // close previous listener
        }
                ...
                ref2.getDocuments { querySnapshot, error in
                    ...
                    for (index, userID) in allUniqueUsersIDsToBeHydrated.enumerated() {
                        if self.isFriendsUpdateNeeded {
                            let friendListener = usersRef.document(userID).addSnapshotListener { document, _ in
                                if let userDict = document?.data() {
                                    let user = GWSUser(representation: userDict)
                                    serialQueue.sync {
                                    ...
                                    if hydratedUsers.count == allUniqueUsersIDsToBeHydrated.count {
                                        ...
                                        let friendships = friendsIDs.compactMap { friendID -> GWSChatFriendship? in
                                            if let otherUser = self.user(for: friendID, in: hydratedUsers) {
                                                return GWSChatFriendship(currentUser: viewer, otherUser: otherUser, type: .mutual)
                                            }
                                            return nil
                                        }
                                        // Then we add the inbound requests
                                        let inboundRequests = inboundRequestsUserIDs.compactMap { userID -> GWSChatFriendship? in
                                            if let otherUser = self.user(for: userID, in: hydratedUsers) {
                                                return GWSChatFriendship(currentUser: viewer, otherUser: otherUser, type: .inbound)
                                            }
                                            return nil
                                        }
                                        // Then we add the outbound requests
                                        let outboundRequests = outboundRequestsUserIDs.compactMap { userID -> GWSChatFriendship? in
                                            if let otherUser = self.user(for: userID, in: hydratedUsers) {
                                                return GWSChatFriendship(currentUser: viewer, otherUser: otherUser, type: .outbound)
                                            }
                                            return nil
                                        }
                            ...
                            usersRef.document(userID).getDocument(completion: { document, _ in
                                if let userDict = document?.data() {
                                    let user = GWSUser(representation: userDict)
                                    serialQueue.sync {
                                        hydratedUsers.append(user)
    ...
    private func user(for id: String, in users: [GWSUser]) -> GWSUser? {
        for user in users {
            if user.uid == id {
                return user
            }
    ...
    func cancelFriendRequest(viewer: GWSUser, to user: GWSUser, completion: @escaping () -> Void) {
        ...
        friendshipRef.getDocuments { snapshot, _ in
            guard let snapshot = snapshot else {
                completion()
                return
            }
            snapshot.documents.forEach { document in
                Firestore.firestore().collection("friendships").document(document.documentID).delete(completion: { _ in
                    self.postGraphUpdateNotification()
                })
            }
            completion()
