...
//  ListingApp
//
//  Created by Jared Sullivan and Florian Marcu on 6/10/18.
...
extension String {
    ...
    func htmlToAttributedString(textColor: UIColor) -> NSAttributedString {
        ...
        do {
            ...
            var modifiedFont = replacingOccurrences(of: "<font color='#007AFF'>", with: "Lawn and OrderHtmlStringTag1")
            modifiedFont = modifiedFont.replacingOccurrences(of: "</font>", with: "Lawn and OrderHtmlStringTag2")
            modifiedFont = modifiedFont.replacingOccurrences(of: "<", with: "&lt;")
            modifiedFont = modifiedFont.replacingOccurrences(of: "Lawn and OrderHtmlStringTag1", with: "<font color='#007AFF'>")
            modifiedFont = modifiedFont.replacingOccurrences(of: "Lawn and OrderHtmlStringTag2", with: "</font>")
...
extension NSAttributedString {
    func fetchAttributedText(allTagUsers: [String]) -> String {
        ...
        for (index, str) in string.enumerated() {
            let strr = attributes(at: index, effectiveRange: nil)
            let checkBlue = "\(strr[NSAttributedString.Key(rawValue: "NSColor")])"
                ...
                if index == string.count - 1 {
                    if !isPreviuosAttrbiutedString.isEmpty {
                        let filterTagUser = allTagUsers.filter { str -> Bool in
                            isPreviuosAttrbiutedString.levenshteinDistanceScore(to: str, ignoreCase: false, trimWhiteSpacesAndNewLines: true) == 1.0
                        }.first
                        if let filterTagUser = filterTagUser {
                            ...
                            let filterTagUser = allTagUsers.filter { str -> Bool in
                                checkIsPreviuosAttrbiutedString.levenshteinDistanceScore(to: str, ignoreCase: false, trimWhiteSpacesAndNewLines: true) == 1.0
                            }.first
                            if let filterTagUser = filterTagUser {
