...
//  NewsReader
//
//  Created by Jared Sullivan and Florian Marcu on 3/28/17.
...
class TimeFormatHelper {
    ...
    static func chatString(for date: Date) -> String {
        ...
        if calendar.isDateInToday(date) {
            return string(for: date, format: "HH:mm")
        }
        return string(for: date, format: "MMM dd")
    }
    ...
    static func timeAgoString(date: Date) -> String {
        ...
        if minutes < 60 {
            return Int(minutes) == 1 ? String(Int(minutes)) + " minute ago" : String(Int(minutes)) + " minutes ago"
        }
